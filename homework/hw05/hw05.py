...
def is_weight(w):
    ...
    "*** YOUR CODE HERE ***"
    return not is_mobile(w) and not is_side(w)
...
def make_fib():
    ...
    n = 1
    def fib_num():
        nonlocal fib_past2, fib_past1, n
        if n == 1:
            fib = fib_past2
        elif n == 2:
            fib = fib_past1
        else:
            fib = fib_past1 + fib_past2
            fib_past2 = fib_past1
            fib_past1 = fib
        n += 1
        return fib
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, try_password):
        nonlocal balance
        if try_password == password and len(error_password) < 3:
            if amount > balance:
                return 'Insufficient funds'
            balance -= amount
            return balance
            ...
            if len(error_password) == 3:
                return "Your account is locked. Attempts: " + str(error_password)
            elif try_password not in error_password:
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    outcome = withdraw(0, old_password)
    if type(outcome) == str:
        return outcome
...
def quadratic(x, a, b, c):
    ...
    if a > 0:
        if upper_bound(x) <= extreme:
            return interval(quad(upper_bound(x)), quad(lower_bound(x)))
        elif lower_bound(x) >= extreme:
            return interval(quad(lower_bound(x)), quad(upper_bound(x)))
        else:
            return interval(quad(extreme), max(quad(lower_bound(x)), quad(upper_bound(x))))
    elif a < 0:
        if upper_bound(x) <= extreme:
            return interval(quad(lower_bound(x)), quad(upper_bound(x)))
        elif lower_bound(x) >= extreme:
            ...
            return interval(min(quad(lower_bound(x)), quad(upper_bound(x))), quad(extreme))
...
def polynomial(x, c):
    ...
    def add_dfn(coeff, k, df):
        return lambda x: coeff * k * pow(x, k - 1) + df(x)
    ...
    extremes = [find_zero(df, ddf, n) for n in starts]
    ns = [n for n in extremes if lower <= n <= upper] + [lower, upper]
