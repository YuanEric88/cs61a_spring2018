...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for col in range(max_cols):
            row_cur += get_piece(board, row, col) + " "
        print(row_cur.strip())
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    target_row, win_target = "", ""
    for col in range(max_cols):
        target_row += get_piece(board, row, col)
    # target_row = [get_piece(board, row, col) for col in range(max_cols)]
    # target = [player for _ in range(num_connect)]
    for _ in range(num_connect):
        win_target += player
    if win_target in target_row:
        return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for row in range(max_rows):
        target_col += get_piece(board, row, col)
    # target_row = [get_piece(board, row, col) for col in range(max_cols)]
    # target = [player for _ in range(num_connect)]
    for _ in range(num_connect):
        win_target += player
    if win_target in target_col:
        return True
    return False
